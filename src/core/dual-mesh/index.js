...
import Delaunator from 'delaunator' // ISC licensed
...
function checkTriangleInequality ({ _r_vertex, _triangles, _halfedges }) {
  ...
  for (let s = 0; s < _triangles.length; s++) {
    ...
    const r1 = _triangles[s_next_s(s)]
    const r2 = _triangles[s_next_s(s_next_s(s))]
    const p0 = _r_vertex[r0]
    const p1 = _r_vertex[r1]
    const p2 = _r_vertex[r2]
...
function addGhostStructure ({ _r_vertex, _triangles, _halfedges }) {
  ...
  let numUnpairedSides = 0; let firstUnpairedEdge = -1
  const r_unpaired_s = [] // seed to side
