...
class TriangleMesh {
  ...
  _update () {
    ...
    for (let s = 0; s < _triangles.length; s += 3) {
      const t = s / 3
      const a = _r_vertex[_triangles[s]]
      const b = _r_vertex[_triangles[s + 1]]
      const c = _r_vertex[_triangles[s + 2]]
      if (this.s_ghost(s)) {
        // ghost triangle center is just outside the unpaired side
        const dx = b[0] - a[0]; const dy = b[1] - a[1]
        const scale = 10 / Math.sqrt(dx * dx + dy * dy) // go 10units away from side
